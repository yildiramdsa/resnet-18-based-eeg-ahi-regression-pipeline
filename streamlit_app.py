...
def show_introduction():
    ...
    with col2:
        ...
        **Sleep apnea** is a common **sleep disorder in which the upper airway repeatedly narrows or collapses during sleep, causing pauses in breathing (apneas) or shallow breathing episodes (hypopneas)**. These interruptions fragment sleep and reduce blood oxygen levels, leading to **daytime fatigue**, **morning headaches**, **impaired concentration**, and an **increased risk of cardiovascular and metabolic disorders**. The **severity of sleep apnea** is quantified by the <span class="inline-badge">Apnea-Hypopnea Index (AHI)</span>, which measures the <span class="inline-badge">number of breathing events per hour of sleep</span>. Accurate AHI assessment is crucial for clinical decision-making.
        ...
        To leverage the advantages of spectrogram-based analysis, we developed a deep learning pipeline using the <span class="inline-badge">ResNet-18 architecture</span>. This choice was motivated by several factors: its **proven performance on image classification tasks makes it well-suited for spectrogram analysis**, as spectrograms can be treated as 2D images where spatial patterns correspond to time-frequency relationships. **The residual connections in ResNet-18 help maintain gradient flow during training**, enabling effective learning of complex hierarchical features from the spectrogram data. **With just 18 layers, ResNet‑18 strikes the right balance between expressiveness and regularization**, providing enough capacity to model complex spectrogram features without overfitting. **Its ImageNet pretrained weights further accelerate learning** by adapting broad image representations to the specific task of AHI regression. Finally, **ResNet-18's computational efficiency** makes it suitable for real-time clinical applications, where rapid inference is essential.

        Our approach performs <span class="inline-badge">continuous AHI regression directly from windowed EEG spectrogram images</span>, rather than using categorical classification, which provides more precise severity estimates that are clinically valuable. The pipeline incorporates <span class="inline-badge">subject-stratified training and validation splits</span> to prevent data leakage, <span class="inline-badge">comprehensive quality control</span> to ensure reliable spectrogram generation, <span class="inline-badge">learning-rate warmup and early stopping</span> to optimize training, and <span class="inline-badge">data augmentation</span> to improve generalization. <span class="inline-badge">Window-level predictions are aggregated at the subject level</span> to yield per-individual AHI estimates suitable for clinical screening and research applications.
        """, unsafe_allow_html=True)
