...
def show_literature_review():
    ...
    with col2:
        ...
        **YOLOv8 demonstrates significantly inferior performance compared to both ResNet64 and YOLOv5, achieving the lowest balanced accuracy (72.25% vs 85.50% and 88.75%) and showing substantial challenges in distinguishing between adjacent severity categories.** The model's AUC values (0.89-0.95) and Average Precision values (0.78-0.91) are notably lower than the other models, particularly for mild and moderate cases.
        ...
        Unlike Tanci and Hekim's categorical approach, we built a **continuous AHI regression pipeline using C3/A2 EEG from 100 SHHS-1 subjects**. We **segmented each recording into 30-s windows (50% overlap), filtered epochs by variance (5-200 µV), and converted only high-quality segments into 224×224 Viridis PNG spectrograms via a 256-point Hann-windowed STFT with cohort-wide normalization**. A **ResNet-18 backbone (ImageNet-pretrained) with a custom regression head was trained under subject-stratified 5-fold cross-validation, leveraging learning-rate warmup, early stopping, and data augmentation**. **Window-level predictions were averaged per subject to produce precise AHI estimates**, which we evaluated using RMSE, MAE, and Pearson correlation, yielding more nuanced, clinically relevant severity scores and realistic generalization to new patients
        """, unsafe_allow_html=True)
        col1, col2, col3 = st.columns([1, 8, 1])
        with col2:
