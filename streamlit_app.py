...
def show_methods():
    ...
    with col2:
        ...
        All spectrograms were normalized using ImageNet's mean and standard-deviation statistics and labelled with each subject's overnight AHI. We trained for up to 30 epochs with a batch size of 16 under a five-fold, subject-stratified cross-validation scheme—80 subjects for training and 20 for validation in each fold—to prevent data leakage and ensure realistic performance on unseen individuals. We evaluated our model by computing the subject-level mean squared error (MSE) and mean absolute error (MAE) between the averaged predictions and the true AHI values. This combination of a lightweight yet powerful architecture, robust optimization strategies, and rigorous validation enabled accurate and efficient AHI regression, suitable for prospective clinical deployment.           
        """, unsafe_allow_html=True)
