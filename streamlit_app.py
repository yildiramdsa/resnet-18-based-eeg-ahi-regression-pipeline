...
def show_introduction():
    ...
    with col2:
        ...
        However, raw EEG signals present significant challenges for automated analysis. They are highly sensitive to artifacts (such as muscle activity, eye movements, and electrode noise), require extensive preprocessing, and contain complex temporal dependencies that are challenging for deep learning models to learn directly. Spectrograms address these limitations by transforming the time-domain signal into a time-frequency representation that preserves both temporal and spectral information. This transformation makes sleep apnea-related patterns more visually apparent and computationally tractable, effectively highlighting frequency bands associated with different sleep stages (delta: 0.5-4 Hz, theta: 4-8 Hz, alpha: 8-13 Hz, beta: 13-30 Hz) and revealing disruptions in these patterns caused by apnea events. Additionally, spectrograms are more robust to noise and artifacts, as the frequency domain representation naturally filters out many types of interference.

        To leverage the advantages of spectrogram-based analysis, we developed a deep learning pipeline using the ResNet-18 architecture. This choice was motivated by several factors: its proven performance on image classification tasks makes it well-suited for spectrogram analysis, as spectrograms can be treated as 2D images where spatial patterns correspond to time-frequency relationships. The residual connections in ResNet-18 help maintain gradient flow during training, enabling effective learning of complex hierarchical features from the spectrogram data. ResNet-18's moderate depth (18 layers) provides sufficient representational capacity without overfitting on our limited medical dataset, while the availability of ImageNet pretrained weights enables effective transfer learning, allowing the model to leverage general image recognition capabilities while fine-tuning for the specific task of AHI regression. Finally, ResNet-18's computational efficiency makes it suitable for real-time clinical applications, where rapid inference is essential.

        Our approach performs continuous AHI regression directly from windowed EEG spectrogram images, rather than using categorical classification, which provides more precise severity estimates that are clinically valuable. The pipeline incorporates subject-stratified training and validation splits to prevent data leakage, comprehensive quality control to ensure reliable spectrogram generation, learning-rate warmup and early stopping to optimize training, and data augmentation to improve generalization. Window-level predictions are aggregated at the subject level to yield per-individual AHI estimates suitable for clinical screening and research applications.
        """, unsafe_allow_html=True)
