...
def show_abstract():
    ...
    with col2:
        st.header("Abstract")
        st.markdown("""
        <span class="inline-badge">Background</span> Sleep apnea leads to repeated upper-airway blockages, disrupted sleep, reduced oxygen levels, and increased cardiovascular and metabolic risks. Its severity is measured using the Apnea-Hypopnea Index (AHI), which indicates the number of apneas and hypopneas per hour.
                    
        <span class="inline-badge">Objective</span> To assess whether a lightweight deep-learning model can continuously estimate the overnight apnea-hypopnea index (AHI) from 30-second single-lead EEG spectrograms with a mean absolute error within ± 5 events per hour, which is the precision typically achieved by manual clinical scoring.
                    
        <span class="inline-badge">Methods</span> We analyzed overnight C3-A2 EEG from 100 adults in the Sleep Heart Health Study (median recording length 7.7 h, IQR 7.1-8.2). Subjects were stratified by consensus AHI: healthy (< 5), mild (5-14.9), moderate (15-29.9), severe (≥ 30); the skewed distribution mirrors population prevalence and was retained to evaluate bias (49/30/14/7). Each trace was split into overlapping 30-s "windows" (50 % overlap), yielding ≈ 166,000 samples. Windows with σ ≤ 5 µV or ≥ 200 µV were discarded. Short-time Fourier transforms (256-point FFT, Hann, 128-sample hop) were mapped to 224 × 224 px RGB images (Viridis, cohort-wide dB limits).
                    
        A ResNet-18 pretrained on ImageNet was fine-tuned for regression (Smooth L1 loss; backbone LR 1 × 10<sup>-5</sup>, head 1 × 10<sup>-4</sup>; Adam; weight decay 1 × 10<sup>-5</sup>). Data augmentation comprised random ±20 % brightness/contrast jitter and horizontal flips, reflecting time-reversal invariance. Five-fold, subject-grouped cross-validation prevented leakage. Window-level outputs were averaged per subject to obtain a single AHI prediction. Performance was summarized with mean absolute error (MAE), root-mean-square error (RMSE), and Pearson's r (agreement with linear ground-truth trend).
                    
        <span class="inline-badge">Results</span> The model achieved MAE = 5.2 events per hour and RMSE = 6.8 events per hour across the five cross-validation folds, with r = 0.76 (p < 0.001) between predicted and true AHI. It overestimated very low AHI values (< 5) by 4-8 events per hour and underestimated moderate-to-severe values (≥ 15) by 5-8 events per hour.
                    
        <span class="inline-badge">Conclusions</span> A transfer-learned ResNet-18 applied to single-lead EEG spectrograms achieved a mean absolute error of 5.2 events per hour—just above the study's ± 5-event target—and exhibited systematic over-estimation at very low AHI and under-estimation at moderate to severe AHI. Future work should expand the training dataset and incorporate multimodal physiological signals (e.g., oximetry, airflow) to correct these edge-case biases and improve accuracy across the full severity spectrum.
                    
        <span class="inline-badge">Keywords<span class="inline-badge"> Sleep apnea, EEG spectrograms, deep learning, ResNet-18, AHI regression
